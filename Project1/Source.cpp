...
int dist_to_water(int p_x, int p_y, vector<int> w_x, vector<int> w_y) {
    int dist_all = MAX_value;
    for (int i = 0; i < w_x.size(); i++) {
        if (abs(p_x - w_x[i]) + abs(p_y - w_y[i]) < dist_all) {                         //перебираем коор-ты всех городов и сравниваем расстоние до них с минимальным расстоянием на данной итерации              
            dist_all = abs(p_x - w_x[i]) + abs(p_y - w_y[i]);                           //объявляем найденное расстояние минимальным, если оно меньше минимального.найденного ранее
        }
    }
    return dist_all;
}
...
int main(int argc, char* argv[])                                                         //Это основная (головная) функция программы, она является точкой входа в программу
{
    ...
    if (answer == 1) {                                                                  //если получаем 1, то попадаем сюда
        ...
        while (barrier_x_pos.size()!=count_barrier) {                                       //основываясь на количестве препятствий создаем столько препятствий
            ...
            if (Sboard[bx][by] == '0' && Lboard[bx][by] == '0') {                             //Здесь мы проверяем, не заната ли клетка с такими координатами
    ...
    //Сюда мы попадем если пользователь при выборе режима ввода данных нажмет 2
        ...
        if (!in.is_open()) {                                                                    //проверяем, можно ли открыть это файл (Может быть так, что файдла не существует)  
            ...
            for (int i = 0; i < n; i++) {                                                      //ставим водоемы 
                Lboard[n - 1][i] = 'w';
                Sboard[n - 1][i] = 'w';
                water_x_pos.push_back(n);
                water_y_pos.push_back(i);
    ...
    */
    for (int i = 0; i < point_x_pos.size(); i++) {                                                                  //пробегаемся по всем точкам. Условия рассматриваются ОДНОВРЕМЕННО с помощью логического AND ( логического и)   
        if (
            (dist_to_water(point_x_pos[i], point_y_pos[i], water_x_pos, water_y_pos) <= min_dist_p_to_w) &&
            (dist_to_city(point_x_pos[i], point_y_pos[i], city_x_pos, city_y_pos) <= min_dist_p_to_c) &&            //условие того, что расстояние до города минимально
            ...
            //это сделаю потому, что одна коор-та мождет совпадать, а вторая будет подходить лучше
        {
            ...
            min_dist_p_to_w = dist_to_water(point_x_pos[i], point_y_pos[i], water_x_pos, water_y_pos);
